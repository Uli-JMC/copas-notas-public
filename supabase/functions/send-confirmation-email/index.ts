...
Deno.serve(async (req) => {
  try {
    ...
    const BRAND_LOGO_URL = (Deno.env.get("BRAND_LOGO_URL") || "").trim(); // https://...
    const EMAIL_HEADER_IMAGE_URL = (Deno.env.get("EMAIL_HEADER_IMAGE_URL") || "").trim(); // https://...
    const WHATSAPP_NUMBER = "50688323801"; // sin + para wa.me

    if (!RESEND_API_KEY) return new Response("Missing RESEND_API_KEY", { status: 500 });
    if (!SB_URL) return new Response("Missing SB_URL", { status: 500 });
    if (!SB_SERVICE_ROLE_KEY) return new Response("Missing SB_SERVICE_ROLE_KEY", { status: 500 });

    const body = (await req.json().catch(() => null)) as ReqBody | null;
    if (!body?.registration_id) return new Response("Missing registration_id", { status: 400 });
    ...
    if (!toEmail) return new Response("Registration has no email", { status: 400 });
              ...
              <tr>
                <td style="padding:0 22px 14px 22px;">
                  ...
                  <p style="margin:0 0 10px 0;color:#374151;">
                    Hola <b>${fullName}</b>, ¡gracias por registrarte!
                  </p>

                  <div style="background:#f3f4f6;border:1px solid #e5e7eb;border-radius:14px;padding:14px 14px;margin:12px 0;">
                  ...
                  <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="border-collapse:collapse;">
                    <tr>
                      <td style="padding:6px 0;color:#111827;"><b>Evento:</b> ${eventTitle}</td>
                    </tr>
                    <tr>
                      <td style="padding:6px 0;color:#111827;"><b>Fecha:</b> ${dateLabel}</td>
                    </tr>
                    <tr>
                      <td style="padding:6px 0;color:#111827;"><b>Hora:</b> ${timeRange}</td>
                    </tr>
                    <tr>
                      <td style="padding:6px 0;color:#111827;"><b>Lugar:</b> ${location}</td>
                    </tr>
                    <tr>
                      <td style="padding:6px 0;color:#111827;"><b>Monto total:</b> ${totalText}</td>
                    </tr>
                  </table>
...
`;
    ...
    if (!resendResp.ok) {
      console.log("Resend error:", resendText);
      return new Response(`Resend error (${resendResp.status}): ${resendText}`, { status: 500 });
    }

    console.log("Resend OK:", resendText);
