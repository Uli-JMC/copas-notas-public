...
(function () {
  ...
  function openModal(modal, p) {
    ...
    if (media) {
      // Evita flash: precarga antes de setear background (placeholder -> Supabase)
      const url = p.mediaImg || "";
      ...
      if (!url) {
        ...
        const apply = () => { media.style.backgroundImage = `url("${url}")`; };
        try{
          if (window.ECN && typeof ECN.preloadImage === "function") ECN.preloadImage(url).then(apply);
          else { const img = new Image(); img.decoding="async"; img.onload=apply; img.onerror=()=>{}; img.src=url; }
        }catch(_){ media.style.backgroundImage = `url("${url}")`; }
      }
    }
