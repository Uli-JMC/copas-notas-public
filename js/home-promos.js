...
(function () {
  ...
  function openModal(modal, p) {
    ...
    if (media) {
      const u = p.mediaImg ? String(p.mediaImg).trim() : "";
      if (!u) {
        media.style.backgroundImage = "none";
      } else {
        media.style.backgroundPosition = "center center"; // mobile-friendly
        media.style.backgroundSize = "cover";
        // Warm cache before swap to reduce visible flicker
        try{
          const IMG = (window.ECN && ECN.img) ? ECN.img : null;
          if (IMG) {
            IMG.preconnect(u);
            IMG.preload(u, { timeout: 1400 }).then((ok) => {
              if (ok) media.style.backgroundImage = `url("${u}")`;
            });
          } else {
