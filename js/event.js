...
function renderEvent(ev) {
  ...
  if (heroBg) {
    ...
    heroBg.style.setProperty("--bgimg", "none");
      applyBgStyleWhenReady(heroBg, safeCssUrl(bg), { preloadHint: true, timeoutMs: 12000 });
      }
...
// -----------------------------
function applyBgStyleWhenReady(el, url, opts){
  const node = el;
  const u = (url && String(url).trim()) || "";
  if (!node) return Promise.resolve(false);
  if (!u){ try{ node.style.backgroundImage = ""; }catch(_){}; return Promise.resolve(false); }

  try{ node.classList.add("img-loading"); node.classList.remove("img-loaded"); }catch(_){}

  try{
    const I = window.ECN && window.ECN.images;
    if (I && typeof I.applyBgWhenReady === "function"){
      // usamos una var CSS temporal para no tocar tu CSS
      node.style.setProperty("--ecn_event_bg", "none");
      node.style.backgroundImage = "var(--ecn_event_bg)";
      return I.applyBgWhenReady(node, "--ecn_event_bg", u, opts).then((ok) => {
        try{ node.classList.add("img-loaded"); node.classList.remove("img-loading"); }catch(_){}
        return ok;
      });
    }
  }catch(_){}
  ...
  // fallback
  try{
    node.style.backgroundImage = `url("${u.replace(/"/g,'\\\"')}")`;
