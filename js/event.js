...
function renderEvent(ev) {
  ...
  const heroBg = $("#heroBg");
  if (heroBg) {
    // Centrado en móvil + evita "flash" (placeholder -> supabase)
    heroBg.style.backgroundPosition = "center center";
    heroBg.style.backgroundSize = "cover";
    heroBg.style.backgroundRepeat = "no-repeat";

    const fallback = normalizeImgPath(getDefaultHero());
    const real = ev && ev.img ? normalizeImgPath(ev.img) : null;

    const fUrl = safeCssUrl(fallback);
    heroBg.style.setProperty("--bgimg", `url('${fUrl}')`);
    heroBg.style.backgroundImage = `url('${fUrl}')`;

    if (real && real !== fallback) {
      const rUrl = safeCssUrl(real);
      const apply = () => {
        heroBg.style.setProperty("--bgimg", `url('${rUrl}')`);
        heroBg.style.backgroundImage = `url('${rUrl}')`;
      };

      try {
        if (window.ECN && typeof ECN.preloadImage === "function") {
          ECN.preloadImage(real).then(apply);
        } else {
          const img = new Image();
          img.decoding = "async";
          img.onload = apply;
