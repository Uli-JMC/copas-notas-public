...
function renderEvent(ev) {
  ...
  const heroBg = $("#heroBg");
  if (heroBg) {
    const desired = normalizeImgPath(ev.img || getDefaultHero());
    // keep centered for mobile + avoid visible "swap" by warming cache first
    try{ heroBg.style.backgroundPosition = "center center"; }catch(_){ }
    // Use current bg as placeholder (don't force a different image immediately)
    const apply = (url) => {
      try{ heroBg.style.setProperty("--bgimg", `url('${safeCssUrl(url)}')`); }catch(_){ }
      try{ heroBg.style.backgroundImage = `url('${safeCssUrl(url)}')`; }catch(_){ }
    };
    const current = (heroBg.style.getPropertyValue("--bgimg") || "").trim();
    // If there's no current, apply immediately; otherwise warm + swap once ready
    if (!current) {
      apply(desired);
    } else {
      try{
        ...
        if (IMG) {
          IMG.preconnect(desired);
          IMG.preload(desired, { timeout: 1400 }).then((ok) => { if (ok) apply(desired); });
        } else {
          apply(desired);
        }
      }catch(_){ apply(desired); }
    }
  }
