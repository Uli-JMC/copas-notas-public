...
// -----------------------------
// ULTRAFAST IMAGE APPLY (no flicker)
// -----------------------------
function applySlideBg(slideEl, url, opts){
  try{
    const I = window.ECN && window.ECN.images;
    if (I && typeof I.applyBgWhenReady === "function"){
      return I.applyBgWhenReady(slideEl, "--bgimg", url, opts);
    }
  }catch(_){}
  // Fallback: set directo (puede producir "flash")
  try{
    slideEl.style.setProperty("--bgimg", `url("${String(url).replace(/"/g,'\\\"')}")`);
    slideEl.classList.add("img-loaded");
    slideEl.classList.remove("img-loading");
  }catch(_){}
  return Promise.resolve(true);
}

function warmHeroOrigins(urls){
  try{
    const I = window.ECN && window.ECN.images;
    if (I && typeof I.warmup === "function") I.warmup(urls);
  }catch(_){}
}
...
function renderEmptyState() {
  ...
  slidesEl.innerHTML = `
    <article class="slide" data-bg="${safeCssUrl(heroImg)}" style="--bgimg:none">
  ...
  `;
  if (dotsEl) 
// ✅ Aplica bg SOLO cuando esté lista (evita flash)
try{
  const s = slidesEl.querySelector(".slide[data-bg]");
  if (s){
    const u = s.getAttribute("data-bg");
    applySlideBg(s, u, { preloadHint: true, timeoutMs: 12000 });
  }
}catch(_){}
dotsEl.innerHTML = "";
}
...
function renderSlides() {
  ...
  EVENTS.forEach((ev, i) => {
    ...
    slide.className = "slide img-loading";
    const bgUrl = safeCssUrl(ev.img || getDefaultHero());
    slide.setAttribute("data-bg", bgUrl);
    slide.style.setProperty("--bgimg", "none");
...
// ✅ Preload + aplicar imágenes SIN flash (primero la activa)
try{
  const slides = [...slidesEl.querySelectorAll(".slide[data-bg]")];
  const urls = slides.map(s => s.getAttribute("data-bg")).filter(Boolean);

  // Calienta DNS/TLS y pre-carga suave algunas
  warmHeroOrigins(urls.slice(0, 4));

  const activeSlide = slides[idx] || slides[0];
  if (activeSlide){
    applySlideBg(activeSlide, activeSlide.getAttribute("data-bg"), { preloadHint: true, timeoutMs: 12000 });
  }

  // El resto en idle (no bloquea)
  try{
    const I = window.ECN && window.ECN.images;
    if (I && typeof I.warmup === "function") I.warmup(urls);
  }catch(_){}
