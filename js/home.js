...
function refreshFromStore(opts = { keepToast: true }) {
  EVENTS = getEvents();
    // Preload 1ra imagen del hero para evitar flash (placeholder -> supabase)
    try{
      const firstImg = (EVENTS && EVENTS[0] && EVENTS[0].img) ? EVENTS[0].img : null;
      if (firstImg && window.ECN && typeof ECN.preloadImage === 'function') {
        await ECN.preloadImage(firstImg);
      }
    }catch(_){ }
