...
function refreshFromStore(opts = { keepToast: true }) {
  EVENTS = getEvents();
    ...
    // ---- IMG warmup (less flicker on first paint)
    try{
      const IMG = (window.ECN && ECN.img) ? ECN.img : null;
      if (IMG && Array.isArray(EVENTS) && EVENTS.length){
        const first = EVENTS[0] && EVENTS[0].img;
        const second = EVENTS[1] && EVENTS[1].img;
        if (first) IMG.preconnect(first);
        await Promise.race([
          Promise.all([
            first ? IMG.preload(first,{timeout:1200}) : Promise.resolve(false),
            second ? IMG.preload(second,{timeout:1200}) : Promise.resolve(false)
          ]),
...
(async function init(){
  ...
  setTimeout(() => toast("Bienvenido", "Revisá los próximos eventos."), 800);
    ...
    try{ console.error("[home] init failed:", e); }catch(_){}
    try{
      // ensure loader doesn't stay stuck
      const loader = document.getElementById("homeLoader") || document.querySelector("[data-home-loader]");
