...
// ============================================================
// Image perf: preload 1st hero before rendering slides (reduce flicker)
// - NO await at top level (safe for non-module scripts)
// ============================================================
function renderSlidesFast() {
  try {
    const first = (EVENTS && EVENTS[0] && EVENTS[0].img) ? String(EVENTS[0].img) : "";
    const ECN = window.ECN || {};
    if (first && ECN.preloadImage) {
      // Render after first hero is ready (or after quick timeout)
      let done = false;
  ...
  renderSlidesFast();
      ...
      ECN.preloadImage(first, { timeoutMs: 1800 }).then(go).catch(go);
      // Safety: don't block UI if network is slow
      setTimeout(go, 900);
      return;
    }
  } catch (_) {}
